...
| Senior Staff approver archived or loses designation while staff still point at them? | Clear `seniorStaffId` for affected staff; re-route `pending_senior_staff` reports to Supervisor (or next valid stage). Triggered on archive, HR sync archive, admin permission change, and when Supervisor opens **Team setup**. |
| Supervisor reassigns `seniorStaffId` while reports are in flight? | Re-route `pending_senior_staff` reports to the new approver (or Supervisor if cleared). |
...
- [x] Optional later: "Assign Senior Staff" in Supervisor portal Team tab.
- [x] **Supervisor portal → Team setup:** supervisors designate Senior Staff and assign `seniorStaffId` within their direct group; in-flight senior-staff reports re-route on change.

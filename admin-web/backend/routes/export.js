                    ...
                    for (const date of sortedDates) {
                      ...
                      for (let tripIndex = 0; tripIndex < dayRoutes.length; tripIndex++) {
                        ...
                        try {
                          ...
                          doc.setFontSize(10);
                          doc.setFont('helvetica', 'normal');
                          safeText(`Date: ${formatMapDate(entry.date) || '—'}`, margin, yPos, { maxWidth: pageWidth - margin * 2 });
                          yPos += 12;
                          safeText(`Cost Center: ${costCenter || '—'}`, margin, yPos, { maxWidth: pageWidth - margin * 2 });
                          yPos += 12;
                          safeText(`Starting Location: ${startDisplay || '—'}`, margin, yPos, { maxWidth: pageWidth - margin * 2 });
                          yPos += 12;
                          safeText(`Ending Location: ${endDisplay || '—'}`, margin, yPos, { maxWidth: pageWidth - margin * 2 });
                          yPos += 12;
                          safeText(`Miles Driven (Tracked by GPS): ${entry.miles != null && entry.miles !== '' ? String(entry.miles) : '—'}`, margin, yPos, { maxWidth: pageWidth - margin * 2 });
                          yPos += 18;

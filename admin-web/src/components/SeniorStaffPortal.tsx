...
import {
  ...
  ReviewDialogMode as SeniorReviewDialogMode,
...
const SeniorStaffPortal: React.FC<SeniorStaffPortalProps> = ({ seniorStaffId, seniorStaffName }) => {
  ...
  const [reviewDialogMode, setReviewDialogMode] = useState<SeniorReviewDialogMode | null>(null);
  ...
  // Handler for approving report from StaffPortal — open approve-only dialog
  ...
  // Approval-action handlers are shared with the Supervisor portal; only id/name,
  ...
  const {
    ...
    reviewerId: seniorStaffId,
    reviewerName: seniorStaffName,
    ...
    labels: {
      ...
      signatureVerifyError: 'Could not verify signature. Please try again.',
      approveSuccessAlert: 'Report approved and sent to the next reviewer successfully!',
    },
    teamReports,
    ...
    setReviewComment,
    onRequestRevisionError: (_error, message) => {
      alert(message);
    },
  });
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'grey.50' }}>
      ...
      <ReviewDialog
        ...
        requiresCertification={false}
        labels={{
          approveTitle: 'Approve and advance report',
          approveAlert:
            'This approves the report at your step and moves it forward in the workflow. Use revision if the employee still needs to make changes.',
          certNotRequiredText: 'Weekly check-up reports do not require the certification step.',
          revisionAlert:
            'The employee will get this report back as "needs revision" with the message below.',
          certificationCheckboxLabel: 'Senior staff: I have read and agree to the certification statement above',
          approveButtonLabel: 'Approve and advance report',
        }}
        commentAuthorName={(comment) => comment.seniorStaffName}

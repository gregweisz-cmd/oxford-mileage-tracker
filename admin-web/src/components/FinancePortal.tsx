...
export const FinancePortal: React.FC<FinancePortalProps> = ({ financeUserId, financeUserName }) => {
  ...
  const handleExportToExcel = async (report: ExpenseReport) => {
    try {
      console.log('📊 Exporting report to HTML:', report.id, report.employeeName);
      
      // Generate HTML content matching the Print Preview format
      const printContent = document.getElementById('printable-report');
      if (!printContent) {
        throw new Error('Print preview content not found');
      }
      
      // Create a comprehensive HTML document for export
      ...
      const htmlContent = `
        ...
        <html>
        <head>
          <title>Expense Report - ${report.employeeName} - ${monthNames[report.month - 1]} ${report.year}</title>
          <style>
            @media print {
              @page {
                size: portrait;
      ...
      // Generate filename matching Staff Portal format: LASTNAME,FIRSTNAME EXPENSES MMM-YY.html
      ...
      link.download = `${lastName.toUpperCase()},${firstName.toUpperCase()} EXPENSES ${monthAbbr}-${yearShort}.html`;

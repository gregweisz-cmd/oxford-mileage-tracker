...
function evaluateTiered(
  ...
  if (input.hoursWorked < 8) {
    parts.push('need 8+ hours');
  }
  if (!input.stayedOvernight) {
    parts.push('overnight tier needs stayed overnight');
  }
  if (input.milesTraveled < 100) {
    parts.push('need 100+ miles driven for day tiers');
  }
  if (input.distanceFromBase < 100) {
    parts.push('overnight tier needs 100+ mi from BA');
  }
